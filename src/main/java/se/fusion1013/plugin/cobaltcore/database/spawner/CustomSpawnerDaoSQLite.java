...
public class CustomSpawnerDaoSQLite extends Dao implements ICustomSpawnerDao {
    ...
    public static String SQLiteCreateCustomSpawnerView = "CREATE VIEW IF NOT EXISTS custom_spawners_view AS" +
            " SELECT custom_spawners.custom_spawner_uuid, custom_spawners.entity, custom_spawners.spawner_type, custom_spawners.spawn_count, custom_spawners.activation_range, custom_spawners.spawn_radius, custom_spawners.cooldown, locations.world, locations.x_pos, locations.y_pos, locations.z_pos" +
    ...
    public void saveCustomSpawners(Map<Long, Map<Location, CustomSpawner>> spawners) {
        try {
            ...
            PreparedStatement ps = conn.prepareStatement("INSERT OR REPLACE INTO custom_spawners(custom_spawner_uuid, entity, spawner_type, spawn_count, activation_range, spawn_radius, cooldown) VALUES(?,?,?,?,?,?,?)");
            ...
            for (Map<Location, CustomSpawner> spawnerMap : spawners.values()) {
                for (CustomSpawner spawner : spawnerMap.values()) {
                    ...
                    DataManager.getInstance().getDao(ILocationDao.class).insertLocation(uuid, location);
